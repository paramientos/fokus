<?php
    new class extends Livewire\Volt\Component {
        ...
        public function mount(bool $isEmbedded = false, $projectId = null)
        {
            ...
            $this->loadTasks(); // project_id ayarlandıktan sonra çağrılmalı
        }
        ...
        public function save()
        {
            ...
            try {
                $apiEndpoint = \App\Models\ApiEndpoint::create([
                    'name' => $this->name,
                    'url' => $this->url,
                    'method' => $this->method,
                ...
                $this->dispatch('notify', [
                    'type' => 'success',
                    'message' => 'API endpoint başarıyla oluşturuldu.'
                ]);

                if ($this->isEmbedded) {
                    $this->dispatch('api-endpoint-created', endpointId: $apiEndpoint->id)->to('api-tester.workspace');
                    $this->resetForm();
                } else {
                    return redirect()->route('api-tester.workspace');
                }
            } catch (\Exception $e) {
                $this->dispatch('notify', [
                    'type' => 'error',
        ...
        {
            $this->resetExcept('projects', 'tasks', 'isEmbedded');
            $this->headers = [];
            $this->params = [];
            $this->body = [];
            $this->loadTasks();
...
<div>
    <div class="flex justify-between items-center mb-6">
        @if(!$isEmbedded)
            <div>
                <h1 class="text-2xl font-bold">Yeni API Endpoint</h1>
                <p class="text-gray-500">API endpoint oluştur ve test et</p>
            </div>
            <div>
        ...
        @else
            <h1 class="text-xl font-semibold">Yeni API Endpoint Oluştur</h1>
    ...
    <div class="bg-white rounded-lg {{ $isEmbedded ? '' : 'shadow-sm p-6' }}">
        <form wire:submit="save">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <x-input id="name" label="Endpoint Adı" type="text" wire:model="name" class="w-full" placeholder="Örn: Kullanıcı Listesi" required />
                </div>
                ...
                <div class="md:col-span-2">
                    <x-input label="URL" id="url" type="text" wire:model="url" class="w-full" placeholder="https://api.example.com/users" required />
                </div>

                <div class="md:col-span-2">
                    <x-textarea label="Açıklama" id="description" wire:model="description" class="w-full" placeholder="Bu endpoint hakkında açıklama..." rows="3" />
                </div>

                <div>
                    <x-select label="project_id" id="project_id" wire:model.live="project_id" class="w-full"
                              :options="[
                        ...
                        <div class="mb-3">
                            @foreach($headers as $key => $value)
                                <div class="flex items-center mb-2">
                                    ...
                                    <button type="button" wire:click="removeHeader('{{ $key }}')" class="text-red-500 hover:text-red-700">
                        ...
                        <div class="mb-3">
                            @foreach($params as $key => $value)
                                <div class="flex items-center mb-2">
                                    ...
                                    <button type="button" wire:click="removeParam('{{ $key }}')" class="text-red-500 hover:text-red-700">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </div>
                            @endforeach
                        ...
                        <div class="mb-3">
                            @foreach($body as $key => $value)
                                <div class="flex items-center mb-2">
                                    ...
                                    <button type="button" wire:click="removeBodyField('{{ $key }}')" class="text-red-500 hover:text-red-700">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </div>
                            @endforeach
        ...
        @if($isEmbedded)
            <x-button wire:click="close" variant="ghost" size="sm" class="mt-4">
                <i class="fas fa-times mr-1"></i> Kapat
            </x-button>
        @endif
    </div>
