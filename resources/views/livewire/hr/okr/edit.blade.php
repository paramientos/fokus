...
new class extends Component {
    ...
    public function mount(OkrGoal $goal)
    {
        ...
        $this->goal = $goal;
        $this->employee_id = $goal->employee_id;
        $this->title = $goal->title;
        $this->description = $goal->description;
        $this->type = $goal->type;
        $this->parent_id = $goal->parent_id;
        $this->target_value = $goal->target_value;
        $this->current_value = $goal->current_value;
        $this->unit = $goal->unit;
        $this->start_date = $goal->start_date->format('Y-m-d');
        $this->end_date = $goal->end_date->format('Y-m-d');
        $this->status = $goal->status;
        $this->progress_percentage = $goal->progress_percentage;
        $this->notes = $goal->notes;
        $this->milestones = $goal->milestones ? json_decode($goal->milestones, true) : [];
    }
    ...
    public function save()
    {
        ...
        $this->success('OKR goal updated successfully!');
        return redirect()->route('hr.okr.show', $this->goal);
    }
    ...
    public function with()
    {
        ...
        $parentGoals = OkrGoal::where('workspace_id', $workspaceId)
            ...
            ->where('id', '!=', $this->goal->id) // Exclude current goal
            ->get()
...
<div>
    <x-header title="Edit OKR Goal" separator progress-indicator>
                        ...
                        @foreach($milestones as $index => $milestone)
                            <div class="flex gap-4 items-end">
                                ...
                                <div class="flex items-center">
                                    <x-checkbox
                                        label="Completed"
                                        wire:model="milestones.{{ $index }}.completed"
                                    />
                                </div>
                                <x-button
                                    icon="fas.trash"
