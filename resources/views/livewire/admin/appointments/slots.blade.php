    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl p-6 border border-slate-200">
            ...
            <form wire:submit.prevent="createSlots" class="space-y-4">
                <x-input type="date" label="Tarih" wire:model="date" icon="fas.calendar" required
                ...
                <x-input type="number" label="Slot Süresi (dakika)" wire:model="duration" icon="fas.hourglass" required
                         min="5" max="120"/>
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-xl p-6 border border-slate-200 mb-6">
                <div class="flex justify-between items-center mb-4">
                    <x-button wire:click="prevMonth" class="text-blue-600 hover:text-blue-500 transition-colors" icon="fas.chevron-left" />
                    <h2 class="text-lg font-medium text-slate-700">{{ $monthName }}</h2>

                    <x-button wire:click="nextMonth" class="text-blue-600 hover:text-blue-500 transition-colors" icon="fas.chevron-right"/>
                </div>
                ...
                <div class="grid grid-cols-7 gap-1">
                    ...
                    @foreach($daysInMonth as $day)
                        <div
                            ...
                            @if($day['hasSlots'])
                                <div
                                    class="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 rounded-full bg-blue-500"></div>
                            @endif
                        </div>
                    @endforeach
                </div>
            ...
            @if($selectedDate)
                <div class="bg-slate-50 rounded-xl p-6 border border-slate-200">
                    <h3 class="text-lg font-medium mb-4 text-slate-700">{{ \Carbon\Carbon::parse($selectedDate)->format('d F Y') }}
                        Tarihli Slotlar</h3>
                    ...
                    @if(count($slots) > 0)
                        <div class="space-y-2">
                            @foreach($slots as $slot)
                                <div
                                    class="flex items-center justify-between p-3 rounded-lg {{ $slot->isBooked() ? 'bg-blue-50/70' : 'bg-slate-100' }} border {{ $slot->isBooked() ? 'border-amber-200' : 'border-slate-200' }}">
                                    <div>
