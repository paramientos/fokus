...
class EnsureWorkspaceIsSelected
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->hasHeader('X-Livewire') || $request->routeIs('admin.*') || $request->routeIs('appointments.*') || $request->routeIs('workspaces.*') || $request->routeIs('landing*') || $request->routeIs('logout') ||  $request->routeIs('login') || $request->routeIs('profile.*') ||  $request->routeIs('register')) {
            return $next($request);
        }
